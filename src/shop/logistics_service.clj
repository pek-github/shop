...
(def my-shop "BookShop Inc.")

(defn create-invoice-template
  "creates the template for the invoice"
  [customer timestamp]
  (str "Enterprise " my-shop
       " acknowledges the legal transaction of accepting __cost__ $ for books, from customer "
       customer ". Transaction took place on " timestamp
       ))
...
(defn invoice-service
  ...
  (let [now (java.util.Date.)]
    (do
      (Thread/sleep (utils/random-number 2000 3000))
